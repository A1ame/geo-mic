...
const ParticipantView = ({ socket, peer, userName, onExit }: any) => {
    ...
    socket.on('available-events', (admins: any[]) => {
      ...
      if (isApproved && activeAdminData) {
        ...
        if (current && current.peerId !== activeAdminData.adminPeerId) {
          const updated = { ...activeAdminData, adminPeerId: current.peerId, adminSocketId: current.socketId };
          setActiveAdminData(updated);
          localStorage.setItem('activeAdmin', JSON.stringify(updated));
          if (status === 'on-air') startStreaming(current.peerId);
        }
      }
    });
  ...
  }, [peer, isApproved, activeAdminData?.adminPeerId, status]);
