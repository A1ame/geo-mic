      ...
      <div className="relative flex-grow h-1/2 border-b border-white/5">
        <MapContainer center={coords} zoom={17} className="h-full w-full grayscale-[0.6] contrast-[1.1]">
          ...
          <Circle 
            center={coords} 
            radius={radius} 
            pathOptions={{ color: isEventStarted ? '#6366f1' : '#475569', weight: 2, fillOpacity: 0.1 }}
          />
        </MapContainer>
        ...
        {!isEventStarted && (
          <div className="absolute inset-x-0 bottom-8 flex justify-center z-[1000] px-6">
            <div className="w-full max-w-md bg-slate-900/90 backdrop-blur-2xl p-6 rounded-[2.5rem] border border-white/10 shadow-2xl">
              ...
              <button 
                onClick={startEvent}
                className="w-full py-4 bg-white text-black rounded-2xl font-black uppercase tracking-widest hover:bg-indigo-50 transition-all flex items-center justify-center gap-3 shadow-xl"
              >
                Создать событие
              </button>
            </div>
